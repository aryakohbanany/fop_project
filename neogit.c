...
void make_local_username(char *username){
     if (access("/mnt/d/pwd", F_OK) != -1){
        ...
        rename(".neogit/config_t", ".neogit/config");
    }
...
int run_log_branch(char *branch){
    ...
    for(int i=last_commit_ID; i>0; i--){
        ...
        if(i==1 && strcmp(branch,tmp_branch)!=0 && f==1)  printf("this branch is not existed\n");
    }
    return 0;
}
...
int run_log_since(char *time){
    ...
    for(int i=last_commit_ID; i>0; i--){
        ...
        fclose(file);
        /*int time1;
        char *result_time = strstr(time, ":");
        result_time[3]='\0';
        move_str(result_time,1);
        time1=atoi(result_time);*/
        ...
        double diff = difftime(timeValue2, timeValue1);
        if(diff>0){
        printf("commit %s:\n",str);
        file=fopen(str,"r");
        while (fgets(line, sizeof(line), file) != NULL) {
...
int run_log_author(char *author){
    //find place of neogit
    FILE *file = fopen("/mnt/d/pwd","r");
    if (file == NULL) return 1;
    char pwd[1000];
    ...
    for(int i=last_commit_ID; i>0; i--){
        ...
        if(i==1 && strcmp(author,tmp_name)!=0 && f==1)  printf("this author is not existed\n");
    }
    return 0;
}
